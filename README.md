...
- Event cancellation sends a real email via SendGrid (`src/lib/sendgrid.ts`), and attendee OTP verification emails go through Supabase's custom SMTP, also pointed at SendGrid.
- **Known email deliverability limitation:** the sending address is only Single Sender–verified in SendGrid, not backed by a domain-authenticated (SPF/DKIM/DMARC) domain, since this project doesn't own a real domain. Major providers with strict DMARC policies — Gmail included — will actively **block** mail from an unauthenticated sender, regardless of Single Sender Verification status (confirmed via SendGrid's Activity Feed: `Processed` at SendGrid, then `Blocked` by `gmail-smtp-in.l.google.com`). In production this would be fixed by verifying a real domain in SendGrid (adding its SPF/DKIM/DMARC DNS records), which gives the sending domain the authentication Gmail requires. For this assignment, expect reliable delivery to non-strict-DMARC inboxes; Gmail recipients may not receive the email even though the app reports success (Supabase's `/auth/v1/otp` and the SendGrid API both return 200 — the request was accepted and handed off correctly; the block happens downstream, entirely outside the app's control).
